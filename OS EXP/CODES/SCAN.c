...
void main()
{
    printf("Niyati Savant--SCAN \n");
    struct track track_order[20];
    int i,j,k,no_tracks,temp,diff=0,x=0,head_movement=0,curr_head,end;
    ...
    printf("Enter the last track number: ");
    scanf("%d",&end);
    track_order[no_tracks].track_no=end;
    track_order[no_tracks].covered=0;
    printf("Enter the cuurent Head position:");
    scanf("%d",&curr_head);
    track_order[no_tracks+1].track_no=curr_head;
    track_order[no_tracks+1].covered=0;
